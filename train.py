...
def train(model, train_dataloader, val_dataloader):
    ...
    best_ssim = config.best_ssim


    for epoch in range(epochs):
        print(' -- Working on training epoch {} --'.format(epoch + 1))
        ...
        for x, y in tqdm(train_dataloader):
            ...
            train_ssim.append(get_SSIM(y, y_hat).item())
            train_psnr.append(psnr(y, y_hat).item())
        ...
        if np.round(np.mean(np.array(train_ssim)), 5) < best_ssim:
            print(f'New best ssim: {np.round(np.mean(np.array(train_ssim)), 5)}, saving model state...')
            best_ssim = np.round(np.mean(np.array(train_ssim)), 5)
            torch.save(model.state_dict(), '/home/plankton/underwater-lucyd/models/lucyd-edof-plankton_best_train_ssim.pth')
        metrics = {
                "train/train_loss": np.mean(np.array(train_loss)),
                "train/train_ssim": np.round(np.mean(np.array(train_ssim)), 5),
                "train/train_psnr": np.round(np.mean(np.array(train_psnr))),
                "train/epoch": epoch + 1,
            }
        wandb.log(metrics)
...
if __name__ == "__main__":
    ...
    config = SimpleNamespace(
        ...
        betas = (0.9, 0.999),
        valid_pct = 0.2,
        start_ckpt = '/home/plankton/underwater-lucyd/models/lucyd-edof-plankton_231204.pth',
        best_loss = 0.02,
        best_ssim = 0.96,
        train_data_version = " 20231204:noise of random strength (between 0.002 and 0.01) in blur, following Tomas' script with torch.poisson noise,added many copepods in synth data"
    )
    model = LUCYD(num_res=1).to(device)
    model.load_state_dict(torch.load(config.start_ckpt))
    data_dir = '/home/plankton/Data/edof_sim_bg_level'
